  ...
  var Employee = React.createClass({
    ...
    handleUpdateRow:function(){
        ...
        if(!/^[a-zA-Z ]+$/.test(getFname) || getFname.length < 2 || getFname.length > 40){
          alert("In Name: Special character and numerics are not allowed. Full Name should be atleast 2 characters");
          return false;
        }

        if(getAge < 0 || getAge > 200){      
          alert("In Age: Please enter age between 0 - 200");
          return false;
        }

        if(!/^[a-zA-Z0-9!@#$ ]+$/.test(getNick) || getNick.length < 2 || getNick.length > 20){
          alert("In Nick Name: Only four special characters[ ! @ # $] are allowed along with alphanumeric. Minimum 2 characters");
          return false;
        }
